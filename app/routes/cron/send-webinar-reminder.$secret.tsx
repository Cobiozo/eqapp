...
export const loader: LoaderFunction = async ({ request, params }) => {
  ...
  // find webinars that starts in one hour and send push notifications to subscribers
  // 1. determine date in 1,05 hour (safe value)
  const dateSafe = new Date();
  dateSafe.setHours(dateSafe.getHours() + 1, 5);
  ...
  for(const web of webinars) {
    ...
    for(const user of users) {
      ...
      await PushNotification(
        request,
        user,
        NotificationContentType.WEBINAR_STARTS_SOON,
        title,
        { webinarId: web.id }
      );

      // if it's external webinar, send mail as well
      if (web.isExternal) {
        ...
        await mailService.sendMail(
          user.email,
          t(user.language.name, "mails.webinarStartsInOneHour.subject"),
          t(user.language.name, "mails.webinarStartsInOneHour.text", web.title[user.language.name] || "", webinarLink)
        );
      }
    }
  }

  // find webinars that starts NOW and send push notifications to subscribers
  // also send mails if it's external webinar (YT)
  // 1. determine safe range for NOW
  const dateSafeOne = new Date();
  dateSafeOne.setMinutes(dateSafeOne.getMinutes() + 5);
  ...
  for(const web of webinarsTwo) {
    ...
    for(const user of users) {
      ...
      await PushNotification(
        request,
        user,
        NotificationContentType.WEBINAR_STARTS_NOW,
        title,
        { webinarId: web.id }
      );
      ...
      if (web.isExternal) {
        ...
        await mailService.sendMail(
          user.email,
          t(user.language.name, "mails.webinarStartsNow.subject"),
          t(user.language.name, "mails.webinarStartsNow.text", web.title[user.language.name] || "", webinarLink)
        );
      }
    }
  }
