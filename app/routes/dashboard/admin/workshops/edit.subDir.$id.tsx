...
export const action: ActionFunction = async ({ request, params }) => {
  ...
  const workshopDir = await workshopDirectoryModel.getById(params.id as string);
  if (!workshopDir)
    return redirect("/404");
  ...
  try {
    ...
    return redirect("/dashboard/admin/workshops/" + workshopDir.parentDirectoryId);
  } catch (error: any) {
    await cleanUp();
...
export const loader: LoaderFunction = async ({ request, params }) => {
  ...
  const workshopDir = await workshopDirectoryModel.getById(params.id as string, ["supportedRoles", "supportedLanguages"]);
  if (!workshopDir)
    return redirect("/404");
...
export default function WorkshopDir() {
  ...
  return (
    <section>
      <AdminPageHeader returnLink={`/dashboard/admin/workshops/${workshopDir.parentDirectoryId}`}>
