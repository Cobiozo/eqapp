...
import { WorkshopItemType, PermissionName } from "@prisma/client";
...
import WorkshopItemFileView from "~/components/layout/WorkshopItemFileView";
...
export const action: ActionFunction = async ({ request , params }) => {
  ...
  // try to find workshop item with this id
  // if not exists, return to workshops
  const id = params.id as string;
  if (!await workshopItemModel.exists({ id }))
  ...
  if (action !== "MOVE")
    return redirect("/dashboard/admin/workshops/edit/item/" + id + "/files");
...
/********* COMPONENT *********/
export default function WorkshopItemFileItem() {
        ...
        >
          ...
          >
            ...
            { user?.permissions[PermissionName.adminPagesEdit] && (
              <Button
                to={`/dashboard/admin/workshops/edit/item/${workshopItemId}/files/create`}
