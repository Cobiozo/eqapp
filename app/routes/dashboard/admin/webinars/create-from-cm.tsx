...
const schema = createFormSchema({
  cmId: {
    type: "number",
    label: "webinars.fields.cmId",
    required: true,
    description: "webinars.fields.cmIdDescription",
  },
  variant: {
    type: "select",
...
export const action: ActionFunction = async ({ request }) => {
  ...
  try {
    ...
    // try to find cm webinar with this id
    const cmWebinar = await cm.getWebinarInfo(data.cmId);
    if (!cmWebinar || !cmWebinar.conference) {
      await cleanUp();
      return BadRequestError({ data: { cmId: "common.validation.notFound" } });
    }

    // check if webinar is in the future
    const startsAt = new Date(cmWebinar.conference.starts_at).toISOString();
    if (new Date(startsAt).getTime() < Date.now()) {
      await cleanUp();
      return BadRequestError({ data: { cmId: "common.validation.dateInPast" } });
    }

    // check if access type if 1 (public) 
    if (cmWebinar.conference.access_type !== CMWebinarAccessType.PUBLIC) {
      await cleanUp();
      return BadRequestError({ data: { cmId: "common.validation.invalidType" } });
    }

    // create webinar record in the database
    const embedUrl = cmWebinar.conference.embed_room_url;
    ...
    // try to enable registration for this webinar
    await cm.enableRegistration(data.cmId);

    // redirect to webinars
    return redirect("/dashboard/admin/webinars?webinarSubscribed=true");
